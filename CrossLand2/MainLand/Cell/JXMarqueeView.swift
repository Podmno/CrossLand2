...
extension UIView: JXMarqueeViewCopyable {
    @objc open func copyMarqueeView() -> UIView {
        ...
        let copyView =  try? NSKeyedUnarchiver.unarchivedObject(ofClass: UIView.self, from: archivedData)
        return copyView ?? UIView()
    }
}
...
public class JXMarqueeView: UIView {
    ...
    fileprivate func startMarquee() {
        ...
        if #available(macCatalyst 15.0, iOS 15, *) {
            self.marqueeDisplayLink?.preferredFrameRateRange = CAFrameRateRange(minimum: 80*Float(self.frameInterval), maximum: 80*Float(self.frameInterval))
        } else {
